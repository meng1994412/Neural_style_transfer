...
class NeuralStyle:
    ...
    def __init__(self, settings):
        ...
        # allocate memory of our output image, then combine the content, style,
        # and output into a single tensor so they can be fed through network
        self.output = K.placeholder((1, self.dims[0], self.dims[1], 3))
        self.input = K.concatenate([self.content, self.style, self.output],
            axis = 0)
        ...
        # extract features from the content layer, then extract the activation
        # from the style image (index 0) and the output image (index 2)
        # these will serve as our style features and output features from the
        # content layer
        ...
        # initialize style loss along with the value used to weight each
        ...
        # the implementation of L-BFGS
        # we create a Keras function that can compute both the loss and
        # gradients together together and then return each separately
        # using two different class methods
        self.lossAndGrads = K.function([self.output], outputs)

    def preprocess(self, p):
        # load the input image (while resizing it to the desired dimensions)
        # and preprocess it
    ...
    def deprocess(self, image):
        # reshape the image, then revserse the zero-centering by adding
    ...
    def gramMat(self, X):
        # the gram matrix is the dot product between the input vector
    ...
    def featureReconLoss(self, styleFeatures, outputFeatures):
        ...
        # between the style features and output features
        return K.sum(K.square(outputFeatures - styleFeatures))

    def styleReconLoss(self, styleFeatures, outputFeatures):
        # compute the style reconstruction loss where A is the gram matrix
        # for the style image and G is the gram matrix for the generated image
        ...
        # return the style reconstruction
        return loss

    def tvLoss(self, X):
        # the total variational loss encourages spatial smoothness in the
        # output page, here we avoid border pixels to avoid artifacts
        (h, w) = self.dims
    ...
    def transfer(self, maxEvals = 20):
        ...
        # array, slowly modified as we run L-BFGS to apply style transfer
        ...
        for i in range(0, self.S["iterations"]):
            ...
            print("[INFO] starting iteration {} of {}...".format(i + 1,
                self.S["iterations"]))
            (X, loss, _) = fmin_l_bfgs_b(self.loss, X.flatten(),
                fprime = self.grads, maxfun = maxEvals)
            print("[INFO] end of iteration {}, loss: {:.4e}".format(i + 1,
                loss))
